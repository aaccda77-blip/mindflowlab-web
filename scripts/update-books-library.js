...
const libPath = path.resolve(__dirname, '..', 'library.html');
let html = fs.readFileSync(libPath, 'utf8');

// 1. Featured Book section markup to insert right before "<!-- 4 Library Categories -->"
...
if (!html.includes('나는 믿는다, 그러나 갇히지 않는다')) {
  html = html.replace('<!-- 4 Library Categories -->', featuredBookMarkup + '\n        <!-- 4 Library Categories -->');
}

// 2. Update status badges in library.html for 31 books to "출간예정"
html = html.replace(/출시 완료 \(발간\)/g, '출간예정 (기획 완료)');
html = html.replace(/42개 장 완제 집필 중/g, '출간예정 (집필 중)');
html = html.replace(/집필 기획 완료/g, '출간예정 (기획 완료)');
html = html.replace(/대통합 최종 완결권/g, '출간예정 (최종 완결)');

// Update header badge for 31 books
html = html.replace('<span>MYEONGSIM LIBRARY · 31 MASTER BOOKS</span>', '<span>MYEONGSIM LIBRARY · 31 MASTER BOOKS (전권 순차 출간예정)</span>');

fs.writeFileSync(libPath, html, 'utf8');
console.log('✓ Updated library.html with featured published book and "출간예정" badges');
